...
int int_cmp(const void *a, const void *b) {
    int int1, int2;
    sscanf(*(char**) a, "%d", &int1);
    sscanf(*(char**) b, "%d", &int2);
...
static int lomuto(void *array, int left, int right, size_t elem_sz, int (*comp) (const void*, const void*)) {
    char** pivot = (char**) array + right; 
    int index = left - 1;

    for (int i = left; i <= right - 1; i++){
        if (comp((char**)array + i, pivot) < 0){
            index++;
            swap((char**)array + index, (char**)array + i, elem_sz);
        }
    }
    swap ((char**)array + (index + 1), (char**)array + right, elem_sz);
    int ret = index + 1;
